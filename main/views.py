...
def pay_success(request):
	...
	from_email='vishuj802@gmail.com'

	msg = EmailMessage(subject, html_content, from_email, ['thevishujadhav@gmail.com'])
...
def user_dashboard(request):
    try:
        current_subscription = models.Subscription.objects.get(user=request.user)
    ...
    if current_subscription:
        enddate = current_subscription.reg_date + timedelta(days=current_subscription.plan.validity_days)
    else:
        enddate = None
    ...
    if request.user.is_authenticated:
        ...
        for d in data:
            try:
                notif_status_data = models.NotifUserStatus.objects.get(user=request.user, notif=d)
                if notif_status_data:
                    notif_status = True
    ...
    available_plans = models.SubPlan.objects.all()
